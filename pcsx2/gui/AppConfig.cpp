...
namespace PathDefs
{
	namespace Base
	{
		...
		const wxDirName& Lua()
        {
            static const wxDirName retval(L"scripts");
            return retval;
        }
...
#define PLUGIN_DIR_str(s) #s
	...
	wxDirName Get( FoldersEnum_t folderidx )
	{
		switch( folderidx )
		{
            ...
            case FolderId_Lua:          return GetLua();
...
bool AppConfig::FolderOptions::IsDefault( FoldersEnum_t folderidx ) const
{
	switch( folderidx )
	{
        ...
        case FolderId_Lua:          return UseDefaultLua;
...
wxDirName GetLuaFolder()
{
    return GetResolvedFolder(FolderId_Lua);
}
...
void AppConfig::FolderOptions::ApplyDefaults()
{
    ...
    if( UseDefaultLua )         Lua           = PathDefs::GetLua();
